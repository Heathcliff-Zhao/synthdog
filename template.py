...
class SynthDoG(templates.Template):
    ...
    def generate(self):
        ...
        bboxes = [l.quad for l in text_layers]
        data = {
            "image": image,
            "label": label,
            "quality": quality,
    ...
    def save(self, root, data, idx):
        ...
        for idx in range(len(bboxes)):
            bboxes[idx] = bboxes[idx].tolist()

        metadata = self.format_metadata(image_filename=image_filename, keys=["texts", "bbox", "roi"], values=[texts, bboxes, roi])
