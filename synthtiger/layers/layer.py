...
class Layer:
    ...
    def output(self, bbox=None, bboxes=None):
        if bbox is None:
            bbox = self.bbox

        image = utils.create_image(bbox[2:])
        bboxes = utils.paste_image(self.image, image, self.quad - bbox[:2], bboxes=bboxes)
        return image, bboxes
...
class Group:
    def __init__(self, obj, bboxes_=None):
        if isinstance(obj, Iterable):
    ...
    def output(self, bbox=None, bboxes=None):
        ...
        for layer in reversed(self.layers):
            if bboxes is not None and cnt == 1:
                bboxes = utils.paste_image(layer.image, image, layer.quad - bbox[:2], bboxes=bboxes)
    ...
    def merge(self):
        image, bboxes = self.output(bboxes=self.bboxes_)
