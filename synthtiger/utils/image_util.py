...
def paste_image(src, dst, quad, mode="normal", bboxes=None):
    ...
    if not all(
        ...
        matrix = cv2.getPerspectiveTransform(origin, quad) # 3x3 matrix
        # if bboxes is not None:
        #     for idx in range(len(bboxes)):
        #         bbox = [
        #                     [bboxes[idx][0], bboxes[idx][1]],
        #                     [bboxes[idx][2], bboxes[idx][1]],
        #                     [bboxes[idx][2], bboxes[idx][3]],
        #                     [bboxes[idx][0], bboxes[idx][3]],
        #                 ]
        #         bbox = np.append(bbox, np.ones((4, 1)), axis=-1).dot(matrix.T)
        #         bboxes[idx] = bbox[..., :2] / bbox[..., 2, np.newaxis]
        #     # # do transform on bboxes, and return same shape
        #     # bboxes = np.array(bboxes, dtype=np.float32) # Nx4x2 matrix
        #     # print(bboxes[0])
        #     # # print(bboxes.shape)
        #     # bboxes = np.append(bboxes, np.ones((len(bboxes), 4, 1)), axis=-1) # Nx4x3 matrix
        #     # bboxes = bboxes.dot(matrix.T) # Nx4x3 matrix
        #     # bboxes = bboxes[..., :2] / bboxes[..., 2, np.newaxis] # Nx4x2 matrix
        #     # import time
        #     # print(bboxes[0])
